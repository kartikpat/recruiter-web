...
function submitAdminLogin(e){return postRequest(baseUrl+"/admin/login",null,e,function(e,n,s){if(e.status&&"success"==e.status){var i=s.getResponseHeader("Set-Token");return Set_Cookie(cookieName,i,1,"/",baseDomainName),pubsub.publish("adminLoginSuccess",e.data)}},function(e,n,s){return pubsub.publish("adminLoginFail",e)})}
var errorResponses={missingEmail:"Please enter the Email address",invalidEmail:"That looks like an invalid email address",missingPassword:"Please enter your password",invalidPassword:"Please enter a valid password",userFail:"Email address does not exist",passwordFail:"Incorrect password",missingParameters:"Oops! Our engineers will fix this shortly. Please try again after sometime.",serviceError:"Oops! Our engineers are working on fixing this, please try again after sometime.",noInternet:"Looks like you are not connected to the internet"},emailRegex=/^[a-zA-Z0-9.!#$%&’*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$/;function Admin(){var r={};return{init:function(){r.email=$("#email"),r.password=$("#password"),r.login=$("#login"),r.errors=$(".error"),r.key=$("#key"),r.resetSuccess=$("#resetSuccess")},getData:function(){return{email:r.email.val(),password:r.password.val()}},validateLogin:function(){return r.errors.text(""),r.email&&r.email.val()?emailRegex.test(r.email.val())?!(!r.password||!r.password.val())||(r.password.next(".error").text(errorResponses.missingPassword),!1):(r.email.next(".error").text(errorResponses.invalidEmail),!1):(console.log(r.email.next(".error")),r.email.next(".error").text(errorResponses.missingEmail),!1)},loginHandler:function(e){r.login.click(e)},errorHandler:function(e){var s="";switch(e.status){case 404:s=errorResponses.userFail;break;case 401:s=errorResponses.passwordFail;break;case 503:s=errorResponses.serviceError;break;case 422:s=errorResponses.missingParameters;break;default:s=errorResponses.noInternet}r.password.next(".error").text(s)},test:function(e){e(r)},successMsg:function(){r.email.addClass("hidden"),r.password.addClass("hidden"),r.login.addClass("hidden"),r.resetSuccess.html("Password Successfully Reset. You can <a class='link-color' href='login'> Login</a> with new password.")}}}
$(document).ready(function(){var a=Admin();a.init(),a.loginHandler(function(n){if(n.preventDefault(),a.validateLogin()){var i=a.getData();submitAdminLogin(i)}});pubsub.subscribe("adminLoginSuccess",function(n,i){window.location.href=staticEndPoints.dashboard}),pubsub.subscribe("adminLoginFail",function(n,i){a.errorHandler(i)})});