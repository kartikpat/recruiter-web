...
function submitForgotPassword(s){postRequest(baseUrl+"/recruiter/forgot-password",null,s,function(s,u,t){if(s.status&&"success"==s.status)return pubsub.publish("successfulForgotPassword",s.data)},function(s,u,t){return pubsub.publish("failedForgotPassword",s)})}
var errorResponses={missingEmail:"Please enter the email address",invalidEmail:"That looks like an invalid email address",userFail:"We could not find an account with that email address.Please register.",missingParameters:"Oops! Our engineers will fix this shortly. Please try again after sometime.",serviceError:"Oops! Our engineers are working on fixing this, please try again after sometime.",noInternet:"Looks like you are not connected to the internet"},emailRegex=/^[a-zA-Z0-9.!#$%&’*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$/;function userForgotPassword(){var s={};return{init:function(){s.email=$("#email"),s.login=$("#login"),s.errors=$(".error"),s.recoverySuccess=$("#recoverySuccess")},getData:function(){return{email:s.email.val()}},validate:function(){return s.errors.text(""),s.recoverySuccess.text(""),s.email&&s.email.val()?!!emailRegex.test(s.email.val())||(s.email.next(".error").text(errorResponses.invalidEmail),!1):(s.email.next(".error").text(errorResponses.missingEmail),!1)},submitHandler:function(e){s.login.click(e)},errorHandler:function(e){var r="";switch(e.status){case 404:r=errorResponses.userFail;break;case 401:case 503:r=errorResponses.serviceError;break;case 422:r=errorResponses.missingParameters;break;default:r=errorResponses.noInternet}s.email.next(".error").text(r)},test:function(e){e(s)},successMsg:function(){s.email.addClass("hidden"),s.login.addClass("hidden"),s.errors.addClass("hidden"),s.recoverySuccess.text("We have sent you a recovery email. Please check your mailbox.")}}}
$(document).ready(function(){var d=userForgotPassword();d.init(),d.submitHandler(function(s){s.preventDefault(),d.validate()&&($("#login").addClass("hidden"),$(".spinner").removeClass("hidden"),submitForgotPassword(d.getData()))});pubsub.subscribe("successfulForgotPassword",function(s,e){$(".button-wrapper").addClass("hidden"),d.successMsg()}),pubsub.subscribe("failedForgotPassword",function(s,e){$("#login").removeClass("hidden"),$(".spinner").addClass("hidden"),d.errorHandler(e)})});
