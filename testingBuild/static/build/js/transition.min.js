...
$(document).ready(function(){setInterval(function(){if("none"==$("#login").css("display"))return $("#wait").addClass("fadeOut"),$("#login").removeClass("fadeOut"),$("#wait").addClass("hidden"),void $("#login").removeClass("hidden");$("#login").addClass("fadeOut"),$("#wait").removeClass("fadeOut"),$("#login").addClass("hidden"),$("#wait").removeClass("hidden")},2e3);pubsub.subscribe("loginVerifySuccess",function(e,i){Set_Cookie(jobseekerCookieName,"",1,"/",baseDomainName),window.location.href=staticEndPoints.dashboard}),pubsub.subscribe("loginVerifyFail",function(e,i){console.log("fail"),Set_Cookie(oldCookieName,"",1,"/",baseDomainName),window.location.href=staticEndPoints.landing});verifyLogin(oldCookieValue)});
...
function verifyLogin(e){postRequest(baseUrl+"/recruiter/login/verify",{},{oldCookie:e},function(e,i,s){if(e.status&&"success"==e.status){var o=s.getResponseHeader("Set-Token");return Set_Cookie(cookieName,o,1,"/",baseDomainName),pubsub.publish("loginVerifySuccess",e)}},function(e,i,s){return e.extraParameters={},pubsub.publish("loginVerifyFail",e)})}