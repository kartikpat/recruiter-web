...
var errorResponses={missingEmail:"Please enter the Email address",invalidEmail:"That looks like an invalid email address",missingPassword:"Please enter your password",invalidPassword:"Please enter a valid password",userFail:"Email address does not exist",passwordFail:"Incorrect password",missingParameters:"Oops! Our engineers will fix this shortly. Please try again after sometime.",serviceError:"Oops! Our engineers are working on fixing this, please try again after sometime.",noInternet:"Looks like you are not connected to the internet"},emailRegex=/^[a-zA-Z0-9.!#$%&’*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$/;function userCredentials(){var s={};return{init:function(){s.email=$("#email"),s.password=$("#password"),s.login=$("#login"),s.errors=$(".error")},getData:function(){return{email:s.email.val(),password:s.password.val()}},validateLogin:function(){return s.errors.text(""),s.email&&s.email.val()?emailRegex.test(s.email.val())?!(!s.password||!s.password.val())||(s.password.next(".error").text(errorResponses.missingPassword),!1):(s.email.next(".error").text(errorResponses.invalidEmail),!1):(console.log(s.email.next(".error")),s.email.next(".error").text(errorResponses.missingEmail),!1)},loginHandler:function(e){s.login.click(e)},errorHandler:function(e){var r="";switch(e.status){case 404:r=errorResponses.userFail;break;case 401:r=errorResponses.passwordFail;break;case 503:r=errorResponses.serviceError;break;case 422:r=errorResponses.missingParameters;break;default:r=errorResponses.noInternet}s.password.next(".error").text(r)},test:function(e){e(s)}}}
$(document).ready(function(){console.log(staticEndPoints);var o=userCredentials();o.init(),o.loginHandler(function(n){n.preventDefault(),o.validateLogin()&&submitLogin(o.getData())});pubsub.subscribe("successfulLogin",function(n,i){window.location=staticEndPoints.dashboard}),pubsub.subscribe("failedLogin",function(n,i){o.errorHandler(i)})});
