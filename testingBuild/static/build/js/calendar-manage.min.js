...
jQuery(".modal-overlay").on("click",".close-modal",function(o){jQuery("body").removeClass("posf"),jQuery(".modal-overlay").addClass("hidden")}),jQuery(".guideline-modal-trigger").on("click",function(){jQuery("body").addClass("posf"),jQuery(".modal-overlay").removeClass("hidden")}),jQuery(".modal-overlay").on("click",function(o){jQuery(o.target).is(".modal-overlay")&&(jQuery(this).addClass("hidden"),jQuery("body").removeClass("posf"))}),jQuery(".modal_overlay").on("click",".close_modal",function(o){jQuery("body").removeClass("posf"),jQuery(".modal_overlay").addClass("hidden")}),jQuery(".modal_overlay").on("click",".close_modal",function(o){jQuery("body").removeClass("posf"),jQuery(".modal_overlay").addClass("hidden")});
...
function fetchRecruiterCalendar(e){return getRequest(baseUrl+"/recruiter/"+e+"/calendar",{},function(e){if(e.status&&"success"==e.status)return pubsub.publish("fetchedCalendars",e)},function(e,u,t){return pubsub.publish("failedToFetchCalendars",e)})}
function Manage(){var r={};return{init:function(){r.rowContainer=$(".schedule-container"),r.rowDate=".calendar-date",r.rowName=".calendar-wrap",r.rowSlots=".total-slots",r.noData=".no-data",r.calendarRowPrototype=$(".calendarRow.prototype"),r.emptyviewPrototype=$(".empty-view.prototype"),r.nocalendarview=$(".no-calendar-view"),r.editSchedule=".edit-schedule",r.wrapperConatiner=$(".wrapper-container"),jQuery(".header .menu-list-item.my-interviews").addClass("active"),$(".createCalendar").click(function(){var e={event_category:eventMap.calendarSetup.cat,event_label:"origin=CalendarEmptyState,recId="+recruiterId};return sendEvent(eventMap.calendarSetup.event,e),!0}),$(".createCalendarNew").click(function(){var e={event_category:eventMap.calendarSetup.cat,event_label:"origin=ManageCalendar,recId="+recruiterId};return sendEvent(eventMap.calendarSetup.event,e),!0})},cloneRow:function(e){if(0==e.length)return r.wrapperConatiner.removeClass("hidden"),$(".container").addClass("hidden"),void $(".page-content").css({"background-color":"#fff",padding:"0%"});e=sortArrayOfObjectsByKeyDescending(e,"timestamp");var n=0;e.forEach(function(e){n++;var t=r.calendarRowPrototype.clone().removeClass("prototype hidden");t.find(r.rowDate).text(moment(e.timestamp).format("MMM DD, YYYY")),t.find(r.rowName).text(e.name).attr("href","/calendar/"+e.id+"/edit");var a=100<e.left?"100+":e.left;t.find(r.rowSlots).text(a),t.find(r.editSchedule).attr("href","/calendar/"+e.id+"/edit"),r.rowContainer.append(t)}),n==e.length&&r.rowContainer.append("<div class='no-data-list'>You have reached the end of the list</div>")}}}
jQuery(document).ready(function(){var r=Manage();r.init(),fetchRecruiterCalendar(recruiterId);pubsub.subscribe("fetchedCalendars",function(e,a){r.cloneRow(a.data)}),pubsub.subscribe("failedToFetchCalendars",function(){alert(res.status)})});
