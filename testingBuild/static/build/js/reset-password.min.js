...
var passKey,email,resetPasswordForm=$(".reset-password-form");$(document).ready(function(){passKey=getUrlParameter("k"),email=getUrlParameter("e"),resetPasswordForm.find(".set-new-password").click(setPassword),resetPasswordForm.find(".field input").focusout(function(){checkErrorClass(this)}),resetPasswordForm.find(".field input").focusin(function(){removeErrorClass(this)})});var checkErrorClass=function(e){var s=$(e),r=s.val();return"newPassword"==s.attr("name")?""==r?(s.next().text("Please provide the password").removeClass("hidden"),!1):r.length<5?(s.next().text("Password must be of atleast five characters").removeClass("hidden"),!1):(s.next().addClass("hidden"),!0):"confirmPassword"==s.attr("name")?""==r?(s.next().text("Please provide the password").removeClass("hidden"),!1):r.length<5?(s.next().text("Password must be of atleast five characters").removeClass("hidden"),!1):$("#newPassword").val()!=$("#confirmPassword").val()?(s.next().text("Password and confirm Password should be same").removeClass("hidden"),!1):(s.next().addClass("hidden"),!0):void 0},removeErrorClass=function(e){$(e).next().addClass("hidden")},getValue=function(e,s){var r=$(e);return s[r.attr("name")]=r.val(),s},setPassword=function(){var e={};resetPasswordForm.find(".field input").each(function(){e=getValue(this,e)}),console.log(e),checkErrorClass($("#newPassword"))&&postRequest(baseUrl+"/recruiter/"+recruiterID+"/set",null,{email:email,key:passKey,password:e.newPassword},successCallback)},successCallback=function(e){"success"==e.status&&(window.location="/sign-in?setNewPassword=1")},getUrlParameter=function(e){var s,r,a=decodeURIComponent(window.location.search.substring(1)).split("&");for(r=0;r<a.length;r++)if((s=a[r].split("="))[0]===e)return void 0===s[1]||s[1]};
function submitResetPassword(s){postRequest(baseUrl+"/recruiter/reset",null,s,function(s,u,e){if(s.status&&"success"==s.status)return pubsub.publish("successfulResetPassword",s.data)},function(s,u,e){return pubsub.publish("failedResetPassword",s)})}
var errorResponses={missingEmail:"Please enter the Email address",invalidEmail:"That looks like an invalid email address",missingPassword:"Please enter your password",missingConfirmPassword:"Please enter confirm password",invalidPassword:"Please enter a valid password",userFail:"Email address does not exist",passwordFail:"Incorrect password",missingParameters:"Oops! Our engineers will fix this shortly. Please try again after sometime.",serviceError:"Oops! Our engineers are working on fixing this, please try again after sometime.",noInternet:"Looks like you are not connected to the internet",passwordMatch:"The new password and confirm password do not match",passwordConflict:"The new password cannot be same as the old password"},emailRegex=/^[a-zA-Z0-9.!#$%&’*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$/;function resetPassword(){var e={};function s(s,r){return a(),!s.val().trim()||!r.val().trim()||!ifBothMatches(s.val(),r.val())&&($(".confirmError").text(errorResponses.passwordMatch),!0)}function a(){e.errors.text("")}return{init:function(){e.email=$(".userEmail"),e.password=$("#password"),e.confirmPassword=$("#passwordConfirm"),e.login=$("#login"),e.errors=$(".error"),e.key=$("#key"),e.resetSuccess=$("#resetSuccess")},getData:function(){return{key:e.key.val(),email:e.email.text(),password:e.password.val()}},validateLogin:function(){return a(),e.password&&e.password.val().trim()?e.confirmPassword&&e.confirmPassword.val().trim()?!s(e.password,e.confirmPassword):(e.confirmPassword.next(".error").text(errorResponses.missingConfirmPassword),!1):(e.password.next(".error").text(errorResponses.missingPassword),!1)},loginHandler:function(s){e.login.click(s)},errorHandler:function(s){var r="";switch(s.status){case 404:r=errorResponses.userFail;break;case 401:r=errorResponses.passwordFail;break;case 409:r=errorResponses.passwordConflict;break;case 503:r=errorResponses.serviceError;break;case 422:r=errorResponses.missingParameters;break;default:r=errorResponses.noInternet}e.confirmPassword.next(".error").text(r)},test:function(s){s(e)},successMsg:function(){e.email.addClass("hidden"),e.password.addClass("hidden"),e.confirmPassword.addClass("hidden"),e.login.addClass("hidden"),e.resetSuccess.html("Password Successfully Reset. You can <a class='link-color' href='login'> Login</a> with new password.")},checkPasswordMatch:s}}
$(document).ready(function(){var r=resetPassword();r.init(),r.loginHandler(function(s){if(s.preventDefault(),r.validateLogin()){var e=r.getData();submitResetPassword(e)}});pubsub.subscribe("successfulResetPassword",function(s,e){r.successMsg()}),pubsub.subscribe("failedResetPassword",function(s,e){r.errorHandler(e)})});
